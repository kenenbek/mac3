...
class Species(Object):
    ...
    def load_from_matrix(self, strain_allele_df, filter_model_genes=True, allele_gene_sep=None):
        ...
        def filter_cobra_model_genes():
            ...
            print("# alleles:", len(strain_allele_df.columns), "-> removing alleles not in GEM -> # alleles after:",
                  len(keep_alleles))
            return keep_alleles
    ...
    def optimize_strains(self, fva_rxn_set="var_reacts", parallel=True, fva=False, fract_opt=0.1,
                         processes=cpu_count()):
        ...
        if self.base_cobra_model:
            ...
            if parallel == True:
                ...
                self.solution = models_optimize_parallel(self, react_set, fract_opt=fract_opt, save_file_loc=None,
                                                         fva=fva, processes=processes)
...
def clean_base_model(mod_, open_exchange=False, verbose=True):
    ...
    check_blcked_reacts = find_blocked_reactions(mod, reaction_list=None, zero_cutoff=1e-6,
                                                 open_exchanges=open_exchange)
    if verbose == True:
        print("# genes=", str(before_mod_genes), "-> removing", len(blcked_reacts), "blocked reactions", "-> # genes=",
              len(mod.genes))
    return mod
    ...
    for all_player in player_list:
        ...
        for react in all_player.cobra_reactions.keys():
            ...
            if max_flux == min_flux:
                ...
                # min_to_mean_grad = np.arange(min_flux, mean_flux, (mean_flux-min_flux)/gradient_steps)
                # max_to_mean_grad = np.arange(mean_flux, max_flux, (max_flux-mean_flux)/gradient_steps)
                min_to_mean_grad = np.arange(min_flux, mean_flux, (mean_flux - min_flux) / (gradient_steps + 1))[
                                   -gradient_steps:]
                max_to_mean_grad = np.arange(mean_flux, max_flux, (max_flux - mean_flux) / (gradient_steps + 1))[
                                   -gradient_steps:]
...
def get_gene_players(gene_list, s_model, verbose=True):
    ...
    if verbose == True:
        print("\t...filtered gene list= (# genes: %d, # alleles: %d, # reactions: %d, # metabolites: %d)" % (
        len(gene_players),
...
from cobrascape.kenenbek import get_input_of_fva, convert_to_graph_and_save
from functools import partial
...
def models_optimize_fva(x, save_samples_dir, t):
    ...
    convert_to_graph_and_save(Species_Object_Global.strains.get_by_id(x).cobra_model, x, save_samples_dir, t,
                              fva_df)
...
def sample_species(model, save_samples_dir, variants, flux_samples, fva_rxn_set="var_reacts", start_samp=None,
    ...
    actions = create_action_set(number_of_actions=action_n,
                                add_no_change=add_na_bound)  ### IMPORTANT- THIS IS UPDATED!!
    ...
    ALLELE_REACT_DF["RXN_DUPS"] = ALLELE_REACT_DF.apply(lambda x: x.value_counts()[x.value_counts() > 1].index.tolist(),
                                                        axis=1)
    ...
    for t in tqdm(range(samples_n)):
        ...
        for strn, allele_row in ALLELE_REACT_DF.iterrows():
            ...
            if len(rxn_dups) > 0:
                ...
                for rxn in rxn_dups:
                    bnd_dict = {"lb": [], "no": [], "ub": []}
                    alleles = {model.alleles.get_by_id(x.split("__")[0]): variant_id_dec_dict[x.split("__")[0]] for x in
                               allele_row[allele_row == rxn].index.tolist()}
                    constraints = [bnd_dict[y.split("_")[0]].append(variant_rxn_action_dict[x][rxn][y]) for x, y in
                                   alleles.items()]
                    if len(bnd_dict["lb"]) > 0:
                        str_obj.cobra_model.reactions.get_by_id(rxn).lower_bound = np.mean(bnd_dict["lb"])
                    else:
                        str_obj.cobra_model.reactions.get_by_id(
                            rxn).lower_bound = model.base_cobra_model.reactions.get_by_id(rxn).lower_bound

                    if len(bnd_dict["ub"]) > 0:
                        str_obj.cobra_model.reactions.get_by_id(rxn).upper_bound = np.mean(bnd_dict["ub"])
                    else:
                        str_obj.cobra_model.reactions.get_by_id(
                            rxn).upper_bound = model.base_cobra_model.reactions.get_by_id(rxn).upper_bound

        # with ProcessPool(processes, initializer=species_init_Objective, initargs=(model,allele_reacts_set,fva_frac_opt,)) as pool:
        with ProcessPool(processes, initializer=species_init_Objective,
                         initargs=(model, popfva_reacts_set, fva_frac_opt)) as pool:
            try:
                if fva == True:
...
def compute_constrained_species(model, var_dec_map, variants, flux_samples, fva_rxn_set="all_reacts",
    ...
    variant_rxn_action_dict = rxn_to_constraints_samples(variants, actions,
                                                         flux_samples)  ### IMPORTANT- THIS IS OLD! v2 available
    ...
    for strn, allele_row in ALLELE_REACT_DF.iterrows():
        ...
        if len(rxn_dups) > 0:
            ...
            for rxn in rxn_dups:
                bnd_dict = {"lb": [], "no": [], "ub": []}
                alleles = {model.alleles.get_by_id(x.split("__")[0]): var_dec_map[x.split("__")[0]] for x in
                           allele_row[allele_row == rxn].index.tolist()}
                constraints = [bnd_dict[y.split("_")[0]].append(variant_rxn_action_dict[x][rxn][y]) for x, y in
                               alleles.items()]
                if len(bnd_dict["lb"]) > 0:
